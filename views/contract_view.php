...
<div id="entryForm" class="position-absolute start-50 translate-middle-x p-3" style="display:none; margin: 5% auto; top:5% ">
    ...
    <form action="#" method="post" name="insert_into_contract" id="contractForm"
        ...
        <div class="card-title text-center fs-3 fw-lighter m-0">Φόρμα Καταχώρησης Σύμβασης</div>  
    ...
    <!---------------------------------------------------------------------------------------------------------------------------------------------->
            <div class="row"> <!--1ο σετ-->
                ...
                <div class="form-floating mb-3 col-lg-6">
                    <input required
                           name="idCONTRACT" id="idCONTRACT" placeholder="Απόφαση Σύμβασης" required 
                           class="form-control" type="text" />
                    <label for="idCONTRACT">Απόφαση Σύμβασης</label>
    ...
    <!---------------------------------------------------------------------------------------------------------------------------------------------->
            <div class="row"> <!--2ο σετ-->   
                ...
                <div class="col-lg-6">
                    <div class="form-floating mb-4">
                        <input name="DATE_OF_DELIVERY" id="DATE_OF_DELIVERY" type="text" onfocus="(this.type='date')"
                               class="form-control" placeholder="Ημερομηνία Παραλαβής" required />
                        <label for="DATE_OF_DELIVERY">Ημερομηνία Παραλαβής</label>
                        <div class="invalid-feedback">Υποχρεωτικό πεδίο</div>
                    </div>
                </div>    
            </div> <!--2ο σετ-->
...
<!-- ALERTS-->
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            type: "POST",
            url: "../handlers/handle_contract_view.php",
                ...
                var table = $('#db_table').DataTable({
                    ...
                    columns: [
                       ...
                       // {data: 'idPINAKA_AWARDING_CONTRACTOR', title: 'idPINAKA_AWARDING_CONTRACTOR', visible: false},
                        ...
                        {data: 'DATE_OF_DELIVERY', title: 'Ημερομηνία Παράδοσης'},
                    ],
        ...
        $('#contractForm').submit(function (event) {
            ...
            $.ajax({
                type: 'POST',
                url: '../handlers/insert_into_contract.php', // Το PHP που διαχειρίζεται τα δεδομένα
                data: formData,
                success: function(response) {
                    if (response.success) {
                    $('#contractForm').trigger('reset');//Επαναφορά της φόρμας
...
$(document).ready(function() {
    ...
    $.get('../handlers/show_decision_to_contract.php', function(options) {
